...
async function getOrCreateBoard(env, boardId) {
  ...
  ).bind(boardId, displayName, null, createdAt, radiusState.currentMeters, JSON.stringify(radiusState), createdAt, "default", 0).run();
  return {
    id: boardId,
  ...
  await env.BOARD_DB.prepare(
    `UPDATE boards
        SET phase_mode = ?1,
            ...
            radius_updated_at = ?5
      WHERE id = ?6`
  ).bind(phaseMode, textOnly ? 1 : 0, nextRadiusMeters, JSON.stringify(nextRadiusState), now, boardId).run();
