...
- Phase-specific environment toggles: `PHASE_ONE_BOARDS`, `PHASE_ONE_TEXT_ONLY_BOARDS`, and `PHASE_ONE_RADIUS_METERS` control fixed-radius/text-only launch behaviour per board.
- Admin API token (`PHASE_ADMIN_TOKEN`) gates phase configuration updates from the dashboard.
- Set `ENABLE_IMAGE_UPLOADS=true` to permit clients to attach pre-uploaded images (validations enforce file type/count/size); leave unset to keep uploads disabled.
